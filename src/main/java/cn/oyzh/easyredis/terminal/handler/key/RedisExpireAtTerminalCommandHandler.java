 ...
 */
@Component
public class RedisExpireAtTerminalCommandHandler extends RedisExpireTerminalCommandHandler {

    @Override
    public TerminalExecuteResult execute(RedisExpireTerminalCommand command, RedisTerminalTextArea terminal) {
        TerminalExecuteResult result = new TerminalExecuteResult();
        try {
            long count = terminal.client().expireAt(null, command.key(), command.time(), command.option());
