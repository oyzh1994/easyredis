 ...
 */
@Component
public class RedisObjectEncodingTerminalCommandHandler extends RedisKeyTerminalCommandHandler<RedisKeyTerminalCommand> {
    ...
    public TerminalExecuteResult execute(RedisKeyTerminalCommand command, RedisTerminalTextArea terminal) {
        ...
        try {
            String objectEncoding = terminal.client().objectEncoding(null, command.key());
            result.setResult(RedisTerminalUtil.formatOut(objectEncoding));
        } catch (Exception ex) {
            ex.printStackTrace();
